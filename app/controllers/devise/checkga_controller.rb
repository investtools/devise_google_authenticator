class Devise::CheckgaController < Devise::SessionsController
  ...
  def update
    ...
    if not resource.nil? || params[:cancel].present?
      ...
      if resource.validate_token(params[resource_name]['gauth_token'].to_i)
        ...
        set_flash_message(:error, :bad_token)
        redirect_to action: :show, id: params[resource_name]['tmpid']
      end

    else
      set_flash_message(:error, params[:cancel].present? ? :canceled : :error)
      redirect_to :root
    end
  end
